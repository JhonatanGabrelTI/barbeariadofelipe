-- Impede a criação de bloqueios em intervalos que já terminaram.
...
begin
    ...
    if v_fim<=now() then
        return jsonb_build_object('success',false,'message','Esse horário já passou. Escolha um intervalo futuro.');
    end if;
